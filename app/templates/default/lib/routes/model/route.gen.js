...
module.exports = function(app, _model){
    ...
    route.owner_query = function(req){
		if(_model.fields.owner){
			if(!_model.fields.owner.bootstrap_populate){
				...
				return {
					owner: req[_model.fields.owner.bootstrap_populate]._id
				};
			}
    ...
    populate:function(req, res, next, id){
		...
		if(_model.parent){
			if(_model.parent == 'owner'){
				...
				if(_model.fields[_model.parent].bootstrap_populate){
					query[_model.parent] = req[_model.fields[_model.parent].bootstrap_populate]._id;
				}
			}
		}
		...
		for(var i = 0; i < req[_model.parent][_model.name.toLowerCase() + 's'].length; i++){
			var m = req[_model.parent][_model.name.toLowerCase() + 's'][i];
			//it is an id
			if(checkForHexRegExp.test(id) && m._id == id){
				model = m;
			} else if(_model.fields.namespace && m.namespace == id){
				model = m;
			}
		}
    ...
    populate_list:function(req, res, next){
        ...
        async.series([
            function(cb){
                if(!_model.is_subdocument){
					app.model[_.capitalize(_model.name)].find(query, function(err, _documents){
						if(err) return next(err);
						documents = _documents;
						res.bootstrap(_model.name + 's', documents);
						return cb();
					});
				} else {
					documents = _.clone(req[_model.parent][_model.name + 's']);
					res.bootstrap(_model.name + 's', documents);
					return cb();
				}
            },
            function(cb){
                ...
                for(var i in documents){
                    var document_data = documents[i].toObject();
                    if(_model.fields.owner){
						if(req.user && (documents[i].owner == req.user._id)){
							document_data._user_is_owner = true;
						}
					}
                    res.locals[_model.name + 's'].push(document_data);
                }
